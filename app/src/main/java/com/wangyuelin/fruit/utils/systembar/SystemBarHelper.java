package com.wangyuelin.fruit.utils.systembar;
...
/**
 * Created by wanghao on 17/5/3 下午6:29.
 * 状态栏工具类
...
public class SystemBarHelper {
    ...
    public static void setStatusBarDarkMode(Activity activity, boolean darkmode) {
        setStatusBarDarkMode(activity.getWindow(), darkmode);
    }
    ...
    public static void setStatusBarDarkMode(Window window, boolean darkmode) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            setStatusBarDarkModeForM(window, darkmode);
        }
        if (isMIUI6Later()) { // 因为MIUI从7.7.13新版本开始已经适配了M。所以为了兼容旧版本
            setStatusBarDarkModeForMIUI6(window, darkmode);
        }
    }
    ...
    public static boolean isFlyme4Later() {
        try {
            ...
            String val = (String) mtd.invoke(null, "ro.build.display.id");
            if (!TextUtils.isEmpty(val)) {
                return val.toLowerCase().contains("flyme");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return result
     */
    public static boolean canStatusDarkMode() {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.M || isMIUI6Later();
    }

    /**
     * 全屏沉浸式，页面布局会渗透到状态栏底部，
     * 因此需要将contentview的padding设置为状态栏高度
     * <4.4 不变
     * >4.4 透明灰色+白色状态栏图标
     * >6.0 全透明+灰色状态栏图标
     */
    public static void setAdaptStatusBar(Window window) {
        if (canStatusDarkMode()) {
            SystemBarHelper.setStatusBarDarkMode(window, true);
            SystemBarHelper.immersiveStatusBar(window, 0);
        } else {
     ...
     * @param window
     */
    public static void setDialogAdapt(Window window) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
